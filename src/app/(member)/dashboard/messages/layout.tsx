...
export default async function MessagesLayout({
  ...
  if (user?.verificationStatus !== "FULLY_VERIFIED") {
    return (
      <div className="flex h-[calc(100vh-10rem)] md:h-[calc(100vh-5.5rem)] -mt-6 -mx-4 md:-mx-8 bg-[var(--surface)] border-t border-[var(--border)] items-center justify-center p-6">
        <div className="max-w-md w-full bg-[var(--surface)] rounded-2xl border border-[var(--border)] shadow-sm p-10 text-center">
          <div className="mx-auto w-14 h-14 rounded-2xl bg-[var(--surface)] border border-[var(--border)] text-[var(--gold-dark)] flex items-center justify-center mb-5 shadow-sm">
            <ShieldAlert size={26} />
          </div>
