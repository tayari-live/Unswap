...
function IncomingCard({ swap }: { swap: SwapRow }) {
  return (
    <div className="bg-[var(--surface)] rounded-[10px] border border-[var(--hair)] shadow-sm overflow-hidden flex flex-col group">
      <div className="p-6">
        <div className="text-[10px] font-bold uppercase tracking-[0.1em] text-[var(--gold-dark)] mb-5">Incoming Request</div>

        <div className="flex items-center gap-3 mb-5">
          <span className="w-10 h-10 rounded-full bg-[var(--navy)]/5 text-[var(--fg)] flex items-center justify-center text-sm font-bold flex-shrink-0">
            {swap.requester.avatarInitials}
          </span>
          <div className="flex flex-col">
            <span className="font-sans text-[15px] font-bold text-[var(--fg)]">{swap.requester.fullName}</span>
            <span className="font-sans text-[13px] text-[var(--fg)]/60">{swap.requester.organisation || "UnSwap Member"}</span>
          </div>
        </div>

        <div className="font-sans text-[14px] text-[var(--fg)]/70 mb-1">wants to stay at</div>
        <h3 className="font-display text-[26px] font-bold text-[var(--fg)] leading-none mb-1">{swap.listing.title}</h3>
        <div className="font-sans text-[14px] text-[var(--fg)]/70 mb-5">{swap.listing.city}, {swap.listing.country}</div>

        <div className="flex flex-col gap-1.5 font-sans text-[14px] text-[var(--fg)] font-medium mb-6">
      ...
      <div className="mt-auto p-4 border-t border-[var(--hair)] bg-[var(--navy)]/5 flex items-center justify-between">
        {/* Gold CTA: navy text on gold is correct in both themes. */}
    ...
    <div className="bg-[var(--surface)] rounded-[10px] border border-[var(--hair)] shadow-sm overflow-hidden flex flex-col group">
      <div className="p-6">
        <div className="flex items-start justify-between mb-5">
          <div className="text-[10px] font-bold uppercase tracking-[0.1em] text-[var(--fg)]/40">Outgoing Request</div>
          <span className={`inline-flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-[0.1em] px-2.5 py-1 rounded-full ${isPending ? "bg-[var(--gold)]/10 text-[var(--gold-dark)]" : "bg-[var(--navy)]/10 text-[var(--fg)]"}`}>
            {isPending && <span className="w-1.5 h-1.5 rounded-full bg-[var(--gold-dark)]" />}
            {swap.status.replace("_", " ")}
          </span>
        </div>

        <h3 className="font-display text-[26px] font-bold text-[var(--fg)] leading-none mb-1">{swap.listing.city || swap.listing.title}</h3>
        <div className="font-sans text-[14px] text-[var(--fg)]/70 mb-5">{swap.listing.country || "Destination"}</div>

        <div className="flex flex-col gap-1.5 font-sans text-[14px] text-[var(--fg)] font-medium mb-6">
          <span className="flex items-center gap-2"><Calendar size={14} className="text-[var(--fg)]/30" /> {fmt(swap.startDate)} – {fmt(swap.endDate)}</span>
          <span className="flex items-center gap-2"><Users size={14} className="text-[var(--fg)]/30" /> {nights(swap.startDate, swap.endDate)} nights</span>
        </div>

        <div className="pt-4 border-t border-[var(--hair)]">
          <div className="font-sans text-[13px] text-[var(--fg)]/60">Host: <span className="font-medium text-[var(--fg)]">{swap.host.fullName}</span></div>
        </div>
      </div>

      <div className="mt-auto p-4 border-t border-[var(--hair)] bg-[var(--navy)]/[0.03] flex items-center gap-2">
        <Link href={`/dashboard/swaps/${swap.id}`} className="flex-1 inline-flex items-center justify-center text-[12px] font-bold uppercase tracking-[0.08em] text-[var(--fg)] bg-[var(--surface)] border border-[var(--border)] hover:bg-[var(--navy)]/5 px-4 py-2.5 rounded transition-colors shadow-sm">
          View Request
        </Link>
        {isPending && (
          <button disabled={busy} onClick={cancelRequest} className="inline-flex items-center justify-center text-[12px] font-bold uppercase tracking-[0.08em] text-[var(--fg)] bg-transparent hover:bg-[var(--navy)]/5 px-4 py-2.5 rounded transition-colors">
            Cancel
          </button>
        )}
      </div>
...
export function SwapsClient({
  ...
  return (
    <>
      ...
      {data.length === 0 ? (
        <div className="bg-[var(--surface)] rounded-lg border border-[var(--hair)] p-12 text-center max-w-2xl mx-auto shadow-sm">
          ...
          <h2 className="font-display text-[32px] font-bold text-[var(--fg)] mb-3 leading-tight">
