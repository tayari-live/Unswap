...
export default async function ExchangeDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="max-w-[800px] mx-auto px-6 lg:px-10 pt-12 pb-32">
      <div className="mb-8">
        <Link href="/dashboard/exchanges" className="inline-flex items-center gap-2 font-sans text-[13px] font-bold uppercase tracking-[0.08em] text-[var(--gold-dark)] hover:text-navy transition-colors">
      ...
      <div className="mb-10">
        ...
        <div className="flex flex-wrap items-center gap-3 font-sans text-[15px] text-navy/70">
          <span className="font-medium text-navy">{fmt(swap.startDate)} — {fmt(swap.endDate)}</span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-12 mb-12 border-b border-[var(--hair)] pb-12">
        <div>
          ...
          <div className="space-y-4 font-sans text-[15px]">
            <div className="flex items-center justify-between">
              <span className="text-navy/60">Check-in</span>
              <span className="font-medium text-navy">{fmt(swap.startDate)}</span>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-navy/60">Check-out</span>
              <span className="font-medium text-navy">{fmt(swap.endDate)}</span>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-navy/60">Duration</span>
              <span className="font-medium text-navy">{n} nights</span>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-navy/60">Credits</span>
              <span className="font-medium text-navy">{swap.mode === "credits" ? n : "Direct Swap"}</span>
            </div>
          </div>
        </div>
        ...
        <div>
          ...
          <div className="bg-[var(--parchment)] rounded-lg p-5">
            <p className="font-sans text-[14px] text-navy/80 mb-4">
            ...
            <div className="flex items-center gap-3">
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="inline-flex flex-1 items-center justify-center gap-2 text-[12px] font-bold uppercase tracking-[0.08em] text-navy bg-white border border-[var(--hair)] hover:bg-navy/5 px-4 py-2.5 rounded transition-colors shadow-sm"
              >
      ...
      {/* Property Guarantee */}
      <div className="mb-12 border-b border-[var(--hair)] pb-12">
        <h3 className="font-sans text-xs font-bold text-navy uppercase tracking-[0.14em] mb-6 flex items-center gap-2">
          <ShieldCheck size={18} className="text-[var(--gold)]" /> Property Guarantee
        </h3>
        <p className="font-sans text-[15px] text-navy/70 leading-relaxed max-w-2xl">
