...
export function PhotoGallery({ photos, title }: { photos: Photo[]; title: string }) {
 ...
 tiles.length > 0 ? "grid-cols-1 sm:grid-cols-4 sm:grid-rows-2" : "grid-cols-1"
        ...
        <button
          type="button"
          onClick={() => show(0)}
          className="absolute bottom-3 right-3 inline-flex items-center gap-1.5 text-xs font-semibold bg-[var(--surface)]/95 text-[var(--fg)] px-3 py-2 rounded-lg shadow hover:bg-[var(--surface)] transition-colors"
        >
