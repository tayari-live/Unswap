...
function SectionLabel({ children, align = "center" }: { children: React.ReactNode; align?: "center" | "left" }) {
  return (
    <div className={`flex items-center gap-3 mb-5 ${align === "center" ? "justify-center" : ""}`}>
      ...
      <span className="text-[11px] tracking-[0.28em] uppercase font-medium text-[#9a7c2c]">{children}</span>
...
const GOLD_BTN =
  "inline-flex justify-center items-center gap-2 text-[13px] font-medium uppercase tracking-[0.1em] text-[#0a0e1a] bg-[#c9a84c] hover:bg-[#e4c97a] px-9 py-4 transition-colors duration-200"
const GHOST_BTN =
  "inline-flex justify-center items-center gap-2 text-[13px] font-medium uppercase tracking-[0.1em] text-white border border-white/30 hover:border-[#c9a84c] hover:text-[#e4c97a] px-9 py-4 transition-colors duration-200"
...
export default function LandingPage() {
  return (
    <div className="bg-[#f5f0e8] text-[#0a0e1a] font-sans">
      ...
      <section className="relative bg-[#0a0e1a] text-white overflow-hidden min-h-[92vh] flex items-center">
        ...
        <div className="relative w-full max-w-[1100px] mx-auto px-6 py-24 flex flex-col items-center text-center mt-8">
          ...
          <div className="mt-16 sm:mt-20 inline-flex flex-col sm:flex-row items-center gap-4 sm:gap-6 text-white/75 text-[13px] font-light">
            <span className="flex items-center gap-2.5"><Globe2 size={17} className="text-[#c9a84c]" /> 100,000+ eligible professionals globally</span>
            <span className="w-px h-4 bg-white/20 hidden sm:block" />
            <span className="flex items-center gap-2.5"><ShieldCheck size={17} className="text-[#c9a84c]" /> Up to $2,000,000 property protection</span>
          </div>
        </div>
      </section>
      ...
      <section id="homes" className="bg-[#0a0e1a] text-white">
        <div className="max-w-[1160px] mx-auto px-6 py-24 lg:py-32">
            ...
            {residences.map((r) => (
              <Link key={r.name} href="/register" className="group relative h-[420px] overflow-hidden border border-white/10 hover:border-[rgba(201,168,76,0.5)] transition-colors duration-300">
                ...
                <div className="absolute inset-x-0 bottom-0 p-8">
                  <span className="text-[10px] font-medium uppercase tracking-[0.22em] text-[#c9a84c]">{r.location}</span>
                  ...
                  <div className="mt-3 flex items-center gap-3 text-[13px] text-white/60">
                    ...
                    <span className="text-[#c9a84c]">{r.duration}</span>
                  </div>
                </div>
              </Link>
            ))}
          ...
          <div className="mt-12 text-center">
            <Link href="/register" className="inline-flex items-center gap-1.5 text-[11px] font-medium uppercase tracking-[0.2em] text-white border-b border-[#c9a84c] pb-1 hover:text-[#e4c97a] transition-colors">
            ...
            {steps.map((s) => (
              <div key={s.n} className="bg-white p-9 lg:p-10 flex flex-col h-full">
                <div className="flex items-center gap-4 w-full">
                  <span className="font-display text-3xl font-light text-[#c9a84c]">{s.n}</span>
                ...
                <div className="mt-auto">
                  <h3 className="font-display text-2xl font-light text-[#0a0e1a] leading-snug">{s.title}</h3>
      ...
      <section className="bg-[#0a0e1a] text-white relative overflow-hidden">
            ...
            <div className="space-y-4 lg:pl-10">
              {protectionCards.map((card) => (
                <div key={card.label} className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white/[0.03] border border-white/10 p-7 backdrop-blur-md">
                  <div>
                    <div className="text-[10px] font-medium uppercase tracking-[0.2em] text-[#c9a84c]/80">{card.label}</div>
                    <div className="text-[22px] font-display font-light mt-1.5 text-white">{card.value}</div>
                  </div>
                  <span className="inline-flex text-[10px] font-medium uppercase tracking-[0.15em] text-[#e4c97a] border border-[rgba(201,168,76,0.3)] px-4 py-2 self-start sm:self-auto">{card.status}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      <section id="pricing" className="bg-[#f5f0e8]">
        <div className="max-w-[1160px] mx-auto px-6 py-24 lg:py-32">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
            {tiers.map((t) => (
              <div key={t.name} className={`relative p-8 flex flex-col bg-white transition-all duration-300 ${t.popular ? "border-2 border-[#c9a84c] shadow-[0_12px_40px_rgba(201,168,76,0.18)] lg:scale-[1.03] z-10" : "border border-[rgba(201,168,76,0.25)] hover:border-[rgba(201,168,76,0.55)]"}`}>
                {t.popular && (
                  <span className="absolute -top-3 left-1/2 -translate-x-1/2 text-[10px] font-medium uppercase tracking-[0.15em] bg-[#c9a84c] text-[#0a0e1a] px-4 py-1.5 whitespace-nowrap">Most Popular</span>
                )}
                ...
                <ul className="mt-6 space-y-3 text-[14px] text-[rgba(10,14,26,0.65)] flex-1">
                  <li className="flex gap-3 font-medium text-[#0a0e1a] items-start"><ShieldCheck size={17} className="text-[#c9a84c] flex-shrink-0 mt-0.5" />{t.exchanges}</li>
                  <li className="flex gap-3 font-medium text-[#0a0e1a] items-start"><ShieldCheck size={17} className="text-[#c9a84c] flex-shrink-0 mt-0.5" />{t.guarantee}</li>
                  {t.features.map((f) => (
                    <li key={f} className="flex gap-3 items-start"><ShieldCheck size={17} className="text-[#c9a84c] flex-shrink-0 mt-0.5" />{f}</li>
                  ))}
                </ul>
                <Link href="/register" className={`mt-8 text-center text-[12px] font-medium uppercase tracking-[0.1em] py-4 transition-colors duration-200 ${t.popular ? "bg-[#c9a84c] text-[#0a0e1a] hover:bg-[#e4c97a]" : "border border-[rgba(10,14,26,0.2)] text-[#0a0e1a] hover:border-[#0a0e1a]"}`}>Request Access</Link>
              </div>
            ))}
          </div>

          {/* Lifetime banner */}
          <div className="mt-8 bg-[#0a0e1a] text-white p-8 lg:p-12 flex flex-col sm:flex-row items-center justify-between gap-8">
            <div className="flex flex-col sm:flex-row sm:items-center gap-6">
              <span className="w-16 h-16 border border-[rgba(201,168,76,0.3)] text-[#c9a84c] flex items-center justify-center shrink-0"><BadgeCheck size={30} /></span>
              <div>
                <div className="flex items-center gap-2">
                  <h3 className="font-display text-xl font-light">Lifetime Access</h3>
                  <span className="text-[9px] font-medium uppercase tracking-[0.12em] bg-[#c9a84c] text-[#0a0e1a] px-2.5 py-1">Best Value</span>
                </div>
        ...
        <div className="max-w-[1160px] mx-auto px-6 pt-20 pb-12">
          <div className="grid gap-12 md:grid-cols-12 lg:gap-8 border-b border-white/10 pb-16">
            ...
            <div className="md:col-span-7 lg:col-span-8 grid grid-cols-2 sm:grid-cols-3 gap-10">
              <div>
                <h4 className="text-[11px] font-medium uppercase tracking-[0.22em] text-[#c9a84c] mb-6">Network</h4>
              ...
              <div>
                <h4 className="text-[11px] font-medium uppercase tracking-[0.22em] text-[#c9a84c] mb-6">Company</h4>
                <ul className="space-y-4 text-[14px] text-white/60 font-light">
              ...
              <div>
                <h4 className="text-[11px] font-medium uppercase tracking-[0.22em] text-[#c9a84c] mb-6">Legal</h4>
                <ul className="space-y-4 text-[14px] text-white/60 font-light">
          ...
          <div className="mt-8 flex flex-col sm:flex-row items-center justify-between gap-6">
            ...
            <div className="flex items-center gap-5 text-white/50">
              <a href="#" aria-label="X (Twitter)" className="p-2 hover:text-[#c9a84c] transition-colors"><svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" /></svg></a>
              <a href="#" aria-label="Facebook" className="p-2 hover:text-[#c9a84c] transition-colors"><svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M22.675 0h-21.35C.597 0 0 .597 0 1.325v21.351C0 23.403.597 24 1.325 24H12.82v-9.294H9.692v-3.622h3.128V8.413c0-3.1 1.893-4.788 4.659-4.788 1.325 0 2.463.099 2.795.143v3.24l-1.918.001c-1.504 0-1.795.715-1.795 1.763v2.313h3.587l-.467 3.622h-3.12V24h6.116c.73 0 1.323-.597 1.323-1.324V1.325C24 .597 23.403 0 22.675 0z"/></svg></a>
              <a href="#" aria-label="LinkedIn" className="p-2 hover:text-[#c9a84c] transition-colors"><svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 01-2.063-2.065 2.064 2.064 0 112.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/></svg></a>
            </div>
          </div>
        </div>
      </footer>
